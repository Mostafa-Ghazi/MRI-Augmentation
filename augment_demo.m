...
Y2 = biasField(X); % image deformation
...
Y4 = motionGhosting(X, alpha, numReps, dim); % image truncation

figure, montage(uint8(Y4), map, 'Indices', [1 : 12]); % truncated image slices
...
figure, montage(Y7, map, 'Indices', [1 : 12]); % noisy image slices
